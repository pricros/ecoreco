...
- (instancetype)initWithFrame:(CGRect)frame WithImageName:(NSString *)iconImageName WithTitle:(NSString *)title{
    ...
    if (self) {
        UIImageView *iconImageview = [[UIImageView alloc]initWithFrame:CGRectMake(10, 5,[UIScreen mainScreen].bounds.size.width/5 , [UIScreen mainScreen].bounds.size.width/5)];
...
- (void)createShareAcitonViewWithFrame:(CGRect)frame WithSourceArray:(NSArray *)arr WithIconArray:(NSArray *)iconArray
{
    ...
    CGFloat nH = 100 + ((self.frame.size.width/5)+ 15) *row;
...
//    self.height = 30 + 80 * row;
    ...
    for (NSInteger p = 0; p < page; p ++) {
        ...
        for (NSInteger i = p*PAGEITEMS; i < arr.count; i ++) {
            if (i < (p+1)*PAGEITEMS) {
                NSInteger column = (i%PAGEITEMS)%3;
                NSInteger r = (i%PAGEITEMS)/3;
